...
module.exports = function startWebpack(runImmediately) {
    ...
    if (runImmediately || (!MODE.local && MODE.production)) {
        ...
        return compiler.run((err, stats) => webpackLogger(LOG, err, stats));
    }

    new TimeFixPlugin().apply(compiler);
        ...
        }),
        {
            ...
            handle: function(req, res, next) {
                ...
                if(req.params.demo === 'index.html') {
                    ...
                    console.log(path.resolve(`${PATHS.folders.dist}, ${req.path}`));
                    const content = require(path.resolve(`${PATHS.folders.dist}, ${req.path}`));
...
function webpackLogger(LOG, err, stats) {
    if (err) {
