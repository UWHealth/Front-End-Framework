...
evaluateTemplatePlugin.prototype.apply = function(compiler) {
    ...
    // Webpack 4+
    if (compiler.hooks) {
        compiler.hooks.compilation.tap('EvalTemplatePlugin', function(
            compilation
        ) {
            compilation.hooks.htmlWebpackPluginBeforeHtmlGeneration.tapAsync(
                'EvalTemplatePlugin',
                (plugin, cb) => {
                    self.init(plugin, cb, compilation);
                }
            );
        });
    }
...
evaluateTemplatePlugin.prototype.init = function(
    htmlPluginData,
    callback,
    compilation
) {
    // eslint-disable-line
    if (
        !htmlPluginData.plugin.options.evalPlugin ||
        !htmlPluginData.plugin.options.evalPlugin.assetName
    ) {
    ...
    this.evalContext = Object.assign(
        {},
        this.evalContext,
        htmlPluginData.plugin.options.evalPlugin.context || {}
    );

    const manifest = this.manifest
        ? this.getSource(this.manifest, compilation, callback)
        : false;
    const source = manifest
        ? manifest + this.getSource(assetName, compilation, callback)
        : this.getSource(assetName, compilation, callback);
    const context = this.evaluateSource(assetPath, source, callback);

    htmlPluginData.plugin.options.evaledTemplate = this.templateHtml(
        htmlPluginData,
        context,
        source,
        compilation,
...
evaluateTemplatePlugin.prototype.templateHtml = function(
    htmlPluginData,
    context,
    source,
    compilation,
    ...
    const templateFn =
        htmlPluginData.plugin.options.evalPlugin.templating || this.templating;

    if (typeof templateFn !== 'function') {
        callback(
            new Error(
                'EvalTemplatePlugin: Templating option must be a function.'
            )
        );
    }
...
evaluateTemplatePlugin.prototype.getSource = function(
    assetName,
    compilation,
    callback
) {
    ...
    try {
        ...
        const source = asset
            ? asset.source()
            : `module.exports = "No asset (${assetName}) found."`;
...
evaluateTemplatePlugin.prototype.evaluateSource = function(
    assetName,
    source,
    callback
) {
