...
Demo_routes.filename = "C:\\Users\\cdl193\\Sites\\Front-End-Framework\\_src\\components\\demo\\demo.routes.html";
...
Demo_routes._render = function(__result, state, options) {
	...
	while (!settled) {
		...
		if (!('pathname' in state)) {
			tmp = Route.data();
			if ('pathname' in tmp) {
				state.pathname = tmp.pathname;
				settled = false;
			}
		}
	...
	return `${Route._render(__result, { pathname: state.pathname, path: state.path }, { store: options.store, slotted: { default: () => `
    ${((state.innerComponent) || __missingComponent)._render(__result, { thisPath: state.pathname }, { store: options.store })}
` } })}`;
};
...
var Route = __webpack_require__(/*! @/components/demo/demo.routes.html */ "../../_src/components/demo/demo.routes.html");
...
Route.filename = "C:\\Users\\cdl193\\Sites\\Front-End-Framework\\node_modules\\svelte-routing\\Route.html";
